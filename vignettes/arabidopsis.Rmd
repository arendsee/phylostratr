...
annotations still refer to the same things. 

```{r, eval=FALSE}
common_ids <- intersect(pstr_strata$qseqid, tips_strata$qseqid)
tips_strata <- dplyr::filter(tips_strata, qseqid %in% common_ids)
pstr_strata <- dplyr::filter(pstr_strata, qseqid %in% common_ids)
# assert both tables have the same number of rows
# you should get 34822 ids
stopifnot(nrow(tips_strata) == nrow(pstr_strata))
```

Now I'll just clean up the tables so the headers and naming conventions match:

```{r, eval=FALSE}
# spaces to underscores
pstr_strata$mrca_name <- sub(' ', '_', pstr_strata$mrca_name)
...
Now merge them into one table

```{r, eval=FALSE}
pstr_strata$group = 'phylostratr'
tips_strata$group = 'tips2014'
all_strata <- rbind(pstr_strata, tips_strata)
```

Next we need to fix the `ps` column, which differs between the two
phylostratigraphs, since the new analysis does not include the Brasicalles
stratum. 

```{r, eval=FALSE}
strata_map <- taxizedb::classification('3702')[[1]]
all_strata <- merge(all_strata, strata_map, by.x='mrca', by.y='id')
all_strata <- dplyr::select(all_strata, qseqid=qseqid, mrca=mrca, mrca_name=name, group=group)
all_strata$mrca_name <- factor(all_strata$mrca_name, levels=strata_map$name)
```
...
```{r, eval=FALSE}
merge(
    subset(strata_summary, group == 'phylostratr'),
    subset(strata_summary, group == 'tips2014'),
    by = 'mrca_name',
    all = TRUE,
    incomparables = 0,
    suffixes = c('.2018', '.2014')
) %>%
    dplyr::select(mrca_name, count.2014, count.2018) %>%
    dplyr::mutate(mrca_name = factor(mrca_name, levels=rev(strata_map$name))) %>%
    dplyr::arrange(mrca_name)
```
