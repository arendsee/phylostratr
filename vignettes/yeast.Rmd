...
saccharomyces <- Strata(
    ...
    focal_name = 'Saccharomyces_cerevisiae',
    focal_id = focal_taxid
)
saccharomyces@tree$tip.label <- taxizedb::name2taxid(saccharomyces@tree$tip.label)
```
...
tree <- prune(strata@tree, '4930', type='name')
# Replace it with out custom tree (4892 is the parent of 4930)
tree <- ape::bind.tree(tree, saccharomyces@tree, where=which(tree_names(tree) == '4892'))
strata@tree <- tree
...
results <- strata_blast(
    query      = strata@data$faa[['Saccharomyces_cerevisiae']],
    strata     = strata,
