...
algo2 <- function(...){
  diverse_subtree(FUN=.algo2, ...)$tip.label
}

test_that("without weights", {
  expect_equal(
    lapply(1:10, function(i) algo2(atree, i)),
    list(
...
test_that("with weights", {
  ...
  expect_equal(algo2(atree, 1, weights=c("t3"=1.001)), "t3")
  # low weight loses to high divesity 
  expect_equal(
    algo2(atree, 2, weights=c("t3"=1.001, "t6"=1.001)),
    c("t6", "t10")
  )
  # but high enough scores are chosen over diversity
  expect_equal(
    algo2(atree, 2, weights=c("t3"=10, "t6"=10)),
    c("t3", "t6")
  )
})
...
test_that("polytomies are well-behaved", {
  ...
  expect_equal(
    lapply(1:7, function(i) algo2(unresolved, i)),
    list(
      c("a"),
      c("a", "e"),
      c("r", "a", "e"),
