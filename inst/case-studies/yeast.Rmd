...
```{r sc_tree1, dpi=300, fig.cap="Step 1: The entire NCBI common tree for all species in the Saccharomycetales family that are represented in UniProt proteomes"}
plot(sc_order@tree, show.node.label=TRUE, cex=0.45, type='cladogram')
```

```{r sc_tree2, dpi=300, fig.cap="Step 2: The Saccharomycetales clade tree after sampling a diverse subset of species from the tree in Step 1"}
plot(sc_filtered@tree, show.node.label=TRUE, cex=0.45, type='cladogram')
```

```{r sc_tree3, dpi=300, fig.cap="Step 3: The tree after replacing the Saccharomyces genus branch with a custom tree"}
plot(sc_final@tree, show.node.label=TRUE, cex=0.45, type='cladogram')
```
