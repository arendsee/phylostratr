...
```{r, dpi=300, fig.cap="Step 1: The NCBI common tree for all species in the Saccharomycetales family that are represented in UniProt proteomes"}
plot(sc_order@tree, show.node.label=TRUE, cex=0.45)
```

```{r, dpi=300, fig.cap="Step 2: The NCBI common tree after sampling a diverse subset of species"}
plot(sc_filtered@tree, show.node.label=TRUE, cex=0.45)
```

```{r, dpi=300, fig.cap="Step 3: The tree after replacing the Saccharomyces branch with a custom tree"}
